...
def gd_step(cost, params, lrate):
    ...
    p = {}
    for param in c:
        p[param] = params[param]-c[param]*lrate
    return p
    ### END CODE
...
class MetaObjective:
    ...
    def __call__(self, params, return_traj=False):
        ...
        trajectory = [params]
        ### YOUR CODE HERE
        test = InnerObjective(self.x,self.y)
        t = params
        for i in range(self.num_steps):
            t = gd_step(test, t, self.inner_lrate)
            trajectory.append(t)
            final_cost = test(t)
        ### END CODE
...
def train():
    ...
    for i in range(OUTER_STEPS):
        ### YOUR CODE HERE
        a,b = data_gen.sample_dataset(NDATA)
        cost = MetaObjective(a, b, OUTER_LRATE, OUTER_STEPS) 
